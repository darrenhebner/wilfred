...
class TwitterProxy {
	 ...
	 * 	@param  string  $retweets
	 */
	public function __construct($oauth_access_token, $oauth_access_token_secret, $consumer_key, $consumer_secret, $screen_name, $count = 5, $retweets) {

		$this->config = array_merge($this->config, compact('oauth_access_token', 'oauth_access_token_secret', 'consumer_key', 'consumer_secret', 'screen_name', 'count', 'retweets'));

		$this->whitelist['statuses/user_timeline.json?' . 'screen_name=' . $this->config['screen_name'] . '&include_rts=' . $this->config['retweets'] . '&count=' . $this->config['count']] = true;
	}
